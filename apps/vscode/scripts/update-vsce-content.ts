...
async function run() {
  ...
  const content = /* ts */`
        ...
        export const VSCE_TRUSTED_SOURCES = ${JSON.stringify(sources, null, 2)}  as const;

        export const VSCE_DEFAULT_IGNORE = ${JSON.stringify(defaultIgnore, null, 2)}  as const;

        export const VSCE_TARGETS = ${JSON.stringify(targets, null, 2)} as const;
    `;
