...
export function loadEmoji(code: string, type?: EmojiType): Promise<Response> {
  ...
  const api = apis[type];
  return fetch(typeof api == "function" ? api(code) : `${api}${code.toUpperCase()}.svg`);
}

export function getIconCode(char: string): string {
  return convertToHexCodePoints(
    !char.includes(ZERO_WITH_JOINER) ? char.replace(VARIATION_SELECTOR_REGEX, "") : char,
  );
}
